    ...
    (
        ...
        $crate::paste::paste! {
            impl $crate::port::PinOps for Dynamic {
                ...
                unsafe fn out_set(&mut self) {
                    match self.port {
                        $(DynamicPort::[<PORT $name>] => (*<$port>::ptr()).outset.write(|w| {
                            w.bits(self.mask)
                        }),)+
                    }
                }
                ...
                unsafe fn out_clear(&mut self) {
                    match self.port {
                        $(DynamicPort::[<PORT $name>] => (*<$port>::ptr()).outclr.write(|w| {
                            w.bits(self.mask)
                        }),)+
                    }
                }
                ...
                unsafe fn out_toggle(&mut self) {
                    match self.port {
                        $(DynamicPort::[<PORT $name>] => (*<$port>::ptr()).outtgl.write(|w| {
                            w.bits(self.mask)
                        }),)+
                    }
                }
            ...
            $($(
                ...
                impl $crate::port::PinOps for [<P $name $pin>] {
                    ...
                    unsafe fn make_input(&mut self, pull_up: bool) {
                        ...
                        if pull_up {
                            (*<$port>::ptr()).[<pin $pin ctrl>].modify(|?, w| {
                                w.pullupen().set_bit()
                            })
                        } else {
                            (*<$port>::ptr()).[<pin $pin ctrl>].modify(|?, w| {
